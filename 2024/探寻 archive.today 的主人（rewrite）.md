...
在 2012 年，这个网站已经有了 10TB 的存档数据，每个月需要花费 300 欧元。这个数字在 2014 年增长到 2000 欧元每月，在 2016 年则来到了4000 欧元每月。在2021年为止，他们已经存档了5 亿个页面，如果按照平均每个页面需要 2MB 来计算，他们需要 1000TB 来存储数据。因为数据存储的成本一直在降，所以我们不能简单地根据用倍数来估算他们现在需要的成本，但参考 *Internet Archive* 的数据量，Internet Archive* 需要 4000TB，可以想象这个成本肯定也非常可观。

但让人有些意外的是，根据网站作者的说法，他们的资源瓶颈不在后端的存储，而在于页面抓取的部分。因为他要做的就是原封不动地获得用户看到的内容，所以这部分没有什么可以取巧的余地。原理上就是，每当用户提交一个网页链接，他就会打开一个浏览器，然后读取并且渲染并保存这个网页内容。听起来是一件挺单纯的事情，但是现在的网页设计越来越复杂，设计标准也越来越繁杂，这导致浏览器已经是一个庞然大物，甚至有观点认为因为要支持的协议和标准变得过于复杂，以至于现在已经不可能从零开始重新创造一款浏览器了。这件事情对于 *archive.today* 的影响就是，为了抓取网页内容，每次运行浏览器都需要大量计算资源。在此之上，他们还准备了一个拥有海量 IP 地址的僵尸网络，通过频换变更爬虫的 IP 地址来对抗反抓取机制。你可能还会好奇，对于一些需要账户登录甚至需要付费才能看到的内容，他们会怎么处理呢？其实也很单纯，就是注册一个账号。当然这种账号也很容易被其他网站侦测到异常，所以常常会被封锁，因此他们有时候也会接受账号的捐赠。
